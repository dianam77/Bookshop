...
namespace Bookshop.Controllers
{
    public class PaymentController : Controller
    {
        ...
        {
            // totalPrice is now populated from the form submission

            // Fetch the basket items asynchronously (if necessary)
            var basketItems = await _orderService.GetUserBasket(User.Identity.Name);

            // (Optional) You can re-calculate totalAmount based on the basket items, 
            // but since you already sent totalPrice from the client, you may not need this
