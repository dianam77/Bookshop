...
namespace Core.BookService
{
    public class BookService:IBookService
    {
        ...
        public async Task UpdateBook(BookDto bookdto)
        {
            ...
            if (bookdto.Img != null)
            {
                // Validate image file size (limit: 200KB)
                long maxFileSize = 200 * 1024; // 200KB limit
                if (bookdto.Img.Length > maxFileSize)
                {
                    throw new ArgumentException("Image size should not exceed 200KB.");
                }

                // Validate file extension (allowed types: .jpg, .jpeg, .png, .gif)
                var allowedExtensions = new List<string> { ".jpg", ".jpeg", ".png", ".gif" };
                var fileExtension = Path.GetExtension(bookdto.Img.FileName).ToLower();
                if (!allowedExtensions.Contains(fileExtension))
                {
                    throw new ArgumentException("Invalid file format. Please upload an image with .jpg, .jpeg, .png, or .gif extension.");
                }

                // If the book already has an image, delete the old one before uploading the new one
                if (!string.IsNullOrEmpty(book.Img))
                {
                    await _fileService.DeleteFile(book.Img);  // Delete old image
                }

                // Upload the new image file
                var result = await _fileService.UploadStaticFile(bookdto.Img, "BookImages");
                book.Img = result.FileAddress;  // Save the new image file path
            }

            // Update the book record in the repository
        ...
        public async Task<PagedBookDto> GetBookPagination(int page, int pageSize, string? search)
        {
            ...
            var bookDto = await books.Select(s => new BookDto()
            {
                ...
                Id = s.Id,
                AuthorName = s.Author != null ? s.Author.Name : "Unknown Author",
                ImgName = s.Img
            }).ToListAsync();
