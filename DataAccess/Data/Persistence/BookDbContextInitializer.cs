...
public class BookDbContextInitializer
{
        ...
        var userPasswords = new Dictionary<string, string>
    {
        ...
        {
            var existingUser = user?.UserName != null ?
                await userManager.FindByNameAsync(user.UserName) : null;
            if (existingUser == null)
            {
                if (user?.UserName != null && userPasswords.TryGetValue(user.UserName, out var password))
                {
                ...
                {
                    // Handle the case where user.UserName is null or userPasswords doesn't contain the key
                    Console.WriteLine($"Error: UserName is null or password for {user?.UserName} not found.");
                }
